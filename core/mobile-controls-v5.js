/* FramePro Mobile Controls V7 — sensores imersivos e centralização completa */
(function(){
'use strict';
...
html.fp-playing #score,html.fp-playing .score,html.fp-playing #fpScore,html.fp-playing #fpMStats,
html.fp-playing [id*="scoreHud"],html.fp-playing [id*="ScoreHud"],html.fp-playing [class*="scoreHud"],html.fp-playing [class*="ScoreHud"],
...
html.fp-web-mobile [id*="HealthHud"],html.fp-web-mobile [id*="healthHud"],html.fp-web-mobile [id*="LifeHud"],html.fp-web-mobile [id*="lifeHud"]{
 position:fixed!important;left:6px!important;top:max(5px,env(safe-area-inset-top))!important;width:114px!important;height:30px!important;max-height:30px!important;min-height:0!important;
...
#fpSensorV4{left:50%!important;bottom:max(12px,env(safe-area-inset-bottom))!important;min-width:146px!important}
#fpCenterV4{left:50%!important;bottom:max(52px,calc(env(safe-area-inset-bottom) + 40px))!important;min-width:112px!important}
html.fp-sensor-on #fpLookV4{opacity:.18!important;pointer-events:none!important}
`;
document.head.appendChild(css);

const heldKeys={KeyW:false,KeyA:false,KeyS:false,KeyD:false};
function keyEvent(code,down){
  const map={KeyW:[87,'w'],KeyA:[65,'a'],KeyS:[83,'s'],KeyD:[68,'d'],KeyX:[88,'x']};
  ...
  if(code in heldKeys)heldKeys[code]=down;
}
function pulseKey(code){keyEvent(code,true);setTimeout(()=>keyEvent(code,false),35);}
function releaseMovement(){Object.keys(heldKeys).forEach(k=>{if(heldKeys[k])keyEvent(k,false);heldKeys[k]=false;});}
...
function applyRoll(delta){
  if(!isFinite(delta)||Math.abs(delta)<.025)return;
  const target=document.querySelector('#gameShell canvas')||document.body;
  target.dispatchEvent(new WheelEvent('wheel',{deltaY:delta>0?26:-26,bubbles:true,cancelable:true}));
}
...
function resetJoystickVisuals(){
  ['fpMoveV4','fpLookV4'].forEach(id=>{const joy=document.getElementById(id),k=joy&&joy.querySelector('.fpKnobV4');if(k)k.style.transform='translate(0px,0px)';});
  releaseMovement();
}
function wireMove(node){
  const joy=cleanOldListeners(node),knob=joy.querySelector('.fpKnobV4');let id=null;
  function set(nx,ny){const d=.15,next={KeyW:ny<-d,KeyS:ny>d,KeyA:nx<-d,KeyD:nx>d};for(const c in next)if(next[c]!==heldKeys[c])keyEvent(c,next[c]);}
  function reset(){id=null;knob.style.transform='translate(0px,0px)';set(0,0);releaseMovement();}
...
function wireLook(node){
  const joy=cleanOldListeners(node),knob=joy.querySelector('.fpKnobV4');let id=null,nx=0,ny=0;
  function loop(){if(id!==null&&!html.classList.contains('fp-sensor-on'))applyLook(nx*2.35,ny*2.05);requestAnimationFrame(loop);}loop();
...
function addFolderButton(){
  ...
  b.onclick=async()=>{const existing=document.getElementById('folderBtn');if(existing){existing.click();return;}if(window.showDirectoryPicker){try{window.FRAMEPRO_PHOTO_DIRECTORY=await window.showDirectoryPicker({mode:'readwrite'});b.textContent='📁 OK';}catch(_){}}else{b.textContent='📁 DOWNLOADS';setTimeout(()=>b.textContent='📁 PASTA',1300);}};
  root.appendChild(b);
}
function repairSensor(){
  const btn=document.getElementById('fpSensorV4');if(!btn||btn.dataset.v7)return;btn.dataset.v7='1';
  const fresh=btn.cloneNode(true);btn.replaceWith(fresh);const center=document.getElementById('fpCenterV4');
  let on=false,lastOri=null,lastMotionAt=0,gotOri=0,gotMotion=0,rollBaseline=null,appliedRoll=0;
  let gyro=null,absoluteSensor=null;
  function norm(v){while(v>180)v-=360;while(v<-180)v+=360;return v;}
  function mappedOrientation(e){
    const angle=(screen.orientation&&screen.orientation.angle)||window.orientation||0;
    const a=e.alpha||0,b=e.beta||0,g=e.gamma||0;
    if(angle===90)return{yaw:-b,pitch:g,roll:-g};
    if(angle===-90||angle===270)return{yaw:b,pitch:-g,roll:g};
    return{yaw:a,pitch:b,roll:g};
  }
  function readOrientation(e){
    if(!on)return;gotOri++;const cur=mappedOrientation(e);
    if(!lastOri){lastOri=cur;if(rollBaseline===null)rollBaseline=cur.roll;return;}
    const dx=norm(cur.yaw-lastOri.yaw),dy=cur.pitch-lastOri.pitch;lastOri=cur;
    if(Math.abs(dx)<14&&Math.abs(dy)<14)applyLook(dx*2.65,dy*2.15);
    const targetRoll=cur.roll-(rollBaseline||0),delta=targetRoll-appliedRoll;
    if(Math.abs(delta)>.55){applyRoll(delta);appliedRoll+=Math.sign(delta)*.7;}
  }
  function readMotion(e){
    if(!on)return;gotMotion++;const rr=e.rotationRate||{};const now=performance.now();if(now-lastMotionAt<18)return;lastMotionAt=now;
    const yaw=(rr.alpha||0)*.018,pitch=(rr.beta||0)*.014;
    if(Math.abs(yaw)<6&&Math.abs(pitch)<6)applyLook(yaw,pitch);
  }
  async function requestPermissions(){
    if(typeof DeviceOrientationEvent!=='undefined'&&typeof DeviceOrientationEvent.requestPermission==='function'){
      const p=await DeviceOrientationEvent.requestPermission();if(p!=='granted')throw new Error('orientação negada');
    }
    if(typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission==='function'){
      const p=await DeviceMotionEvent.requestPermission();if(p!=='granted')throw new Error('movimento negado');
    }
  }
  function startGenericSensors(){
    try{if('Gyroscope'in window){gyro=new Gyroscope({frequency:30});gyro.addEventListener('reading',()=>{if(on&&gyro)applyLook((gyro.y||0)*1.7,(gyro.x||0)*1.45);});gyro.start();}}catch(_){gyro=null;}
    try{if('AbsoluteOrientationSensor'in window){absoluteSensor=new AbsoluteOrientationSensor({frequency:30,referenceFrame:'device'});absoluteSensor.addEventListener('reading',()=>{});absoluteSensor.start();}}catch(_){absoluteSensor=null;}
  }
  function stopGenericSensors(){try{gyro&&gyro.stop();}catch(_){}try{absoluteSensor&&absoluteSensor.stop();}catch(_){}gyro=absoluteSensor=null;}
  function recenterAll(){
    lastOri=null;rollBaseline=null;appliedRoll=0;resetJoystickVisuals();releaseMovement();pulseKey('KeyX');
    ...
    fresh.textContent=on?'📱 SENSORES ON':'📱 ATIVAR SENSORES';
  }
  fresh.onclick=async()=>{try{
    if(typeof DeviceOrientationEvent==='undefined'&&typeof DeviceMotionEvent==='undefined'&&!('Gyroscope'in window))throw new Error('sem suporte');
    await requestPermissions();on=!on;lastOri=null;rollBaseline=null;appliedRoll=0;gotOri=gotMotion=0;
    if(on){
      window.addEventListener('deviceorientation',readOrientation,true);window.addEventListener('deviceorientationabsolute',readOrientation,true);window.addEventListener('devicemotion',readMotion,true);startGenericSensors();
      html.classList.add('fp-sensor-on');fresh.textContent='📱 SENSORES ON';if(center)center.style.display='grid';
      setTimeout(()=>{if(on&&gotOri===0&&gotMotion===0&&!gyro)fresh.textContent='⚠️ SEM DADOS';},2400);
    }else{
      window.removeEventListener('deviceorientation',readOrientation,true);window.removeEventListener('deviceorientationabsolute',readOrientation,true);window.removeEventListener('devicemotion',readMotion,true);stopGenericSensors();
      html.classList.remove('fp-sensor-on');fresh.textContent='📱 ATIVAR SENSORES';if(center)center.style.display='none';resetJoystickVisuals();
    }
  }catch(_){fresh.textContent='⚠️ SENSOR INDISPONÍVEL';}};
  if(center){center.textContent='⟳ CENTRALIZAR TUDO';center.onclick=recenterAll;}
  window.__fpRecenterMobile=recenterAll;
}
function hideCentralPollution(){
  document.querySelectorAll('body *').forEach(el=>{if(el.closest('#start,#feedback,#fpRotateGateV4,#fpFsPromptV4'))return;const id=(el.id||'').toLowerCase(),cl=(el.className||'').toString().toLowerCase();if((id.includes('score')||cl.includes('score'))&&getComputedStyle(el).position==='fixed'){const r=el.getBoundingClientRect();if(r.top>innerHeight*.18&&r.bottom<innerHeight*.82)el.style.setProperty('display','none','important');}});
}
function install(){
  const move=document.getElementById('fpMoveV4'),look=document.getElementById('fpLookV4');if(!move||!look)return false;
  if(!move.dataset.v7){wireMove(move);document.getElementById('fpMoveV4').dataset.v7='1';}
  if(!look.dataset.v7){wireLook(look);document.getElementById('fpLookV4').dataset.v7='1';}
