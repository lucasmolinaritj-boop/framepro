/* FramePro Mobile Sensors V22 — movimento natural, yaw livre, filtro estável e horizonte determinístico */
(function(){
'use strict';
...
const state=window.__fpSensorRuntimeV22=window.__fpSensorRuntimeV22||{enabled:false,paused:false,current:null,last:null,samples:0,centering:false,rollBase:null,appliedRoll:0,smoothYaw:0,smoothPitch:0,lastAction:0};
if(!document.getElementById('fp-sensor-v22-style')){const s=document.createElement('style');s.id='fp-sensor-v22-style';s.textContent=`#fpCenterV4,#fpSensorV4{pointer-events:auto!important;visibility:visible!important}#fpCenterV4{display:grid!important}html.fp-sensor-on #fpLookV4,#fpLookV4{pointer-events:auto!important;opacity:1!important;visibility:visible!important}`;document.head.appendChild(s);}
...
function recalibrate(){state.last=state.current?{...state.current}:null;state.rollBase=state.current?state.current.roll:null;state.smoothYaw=0;state.smoothPitch=0;}
function onOrientation(e){
 ...
 if(Math.abs(dyaw)>18||Math.abs(dpitch)>18){state.smoothYaw=state.smoothPitch=0;return;}
 dyaw=Math.max(-5,Math.min(5,dyaw));dpitch=Math.max(-4,Math.min(4,dpitch));
 state.smoothYaw=state.smoothYaw*.68+dyaw*.32;state.smoothPitch=state.smoothPitch*.72+dpitch*.28;
 if(Math.abs(state.smoothYaw)>.025)trackedLook(-state.smoothYaw*1.55,0);
 if(Math.abs(state.smoothPitch)>.025)trackedLook(0,state.smoothPitch*1.12);
 if(state.rollBase===null)state.rollBase=cur.roll;
 const relRoll=norm(cur.roll-state.rollBase);const wanted=Math.abs(relRoll)<2.2?0:Math.max(-5,Math.min(5,Math.round(relRoll/4.2)));
...
async function toggleSensors(){try{await permission();state.enabled=!state.enabled;state.samples=0;state.current=state.last=null;state.rollBase=null;state.appliedRoll=0;state.smoothYaw=state.smoothPitch=0;cameraOffset.roll=0;bind();refresh();if(state.enabled){toast('Sensores ativados e calibrados');setTimeout(()=>{if(state.enabled&&state.samples===0){bind();const b=document.getElementById('fpSensorV4');if(b)b.textContent='⚠️ SEM DADOS DO SENSOR';toast('O navegador não entregou dados do sensor');}},2800);}else toast('Sensores desligados');}catch(_){state.enabled=false;bind();refresh();const b=document.getElementById('fpSensorV4');if(b)b.textContent='⚠️ SENSOR INDISPONÍVEL';toast('Permissão ou sensor indisponível');}}
...
function resume(){state.paused=false;state.last=null;state.rollBase=null;state.smoothYaw=state.smoothPitch=0;setTimeout(()=>{bind();refresh();},120);setTimeout(()=>{bind();refresh();},650);}
