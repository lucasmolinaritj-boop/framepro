...
const state={enabled:false,paused:false,centering:false,disposed:false,lastAction:0,base:null,current:null,smoothYaw:0,smoothPitch:0,appliedYaw:0,appliedPitch:0,appliedRoll:0,samples:0,lastRawYaw:null,lastRawPitch:null,decoupledYaw:0,verticalLock:0,resumeToken:0};
...
function onOrientation(e){
 ...
 if(!verticalLocked){const wanted=Math.abs(relRoll)<2.1?0:clamp(Math.round(relRoll/4.8),-5,5);if(wanted!==state.appliedRoll){const dir=Math.sign(wanted-state.appliedRoll);wheel(dir>0?11:-11);state.appliedRoll+=dir;cameraOffset.roll=state.appliedRoll;}}
}
...
function suspend(){state.resumeToken++;state.paused=true;state.base=null;state.current=null;state.smoothYaw=state.smoothPitch=0;state.appliedYaw=state.appliedPitch=0;resetAxisHistory();unbind();if(typeof window.__fpResetMobileControls==='function')window.__fpResetMobileControls();}
function resume(){const token=++state.resumeToken;state.paused=false;state.base=null;state.current=null;state.smoothYaw=state.smoothPitch=0;state.appliedYaw=state.appliedPitch=0;resetAxisHistory();[180,700].forEach(delay=>setTimeout(()=>{if(token!==state.resumeToken||state.disposed||document.hidden)return;bind();refresh();},delay));}
